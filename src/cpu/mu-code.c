...
int izx_adr_mode(struct _6510_cpu* cpu, char memory[][9] ){
  int pc = pc2int(cpu);
  int xadr = conv_bitstr2int(cpu->regx,0,7);
  int zp_adr =  conv_bitstr2int(memory[pc],0,7);
  zp_adr = zp_adr+xadr;
  int adr = (conv_bitstr2int(memory[zp_adr+1],0,7) <<8 ) | conv_bitstr2int(memory[zp_adr],0,7);
...
int izy_adr_mode(struct _6510_cpu* cpu, char memory[][9] ){
	int zpl = pc2int(cpu);
	int zpadr = conv_bitstr2int(memory[zpl],0,7) ;
	int lb = conv_bitstr2int(  memory[zpadr],0,7);
	int hb = conv_bitstr2int(  memory[zpadr+1],0,7);

	int regy = conv_bitstr2int(cpu->regy,0,7);
...
int zp_adr_mode(struct _6510_cpu* cpu, char memory[][9] ){
	int eadr = word2int(00000000, cpu->pch);
	int adr = conv_bitstr2int(memory[eadr],0,7) ;
	inc_pc(cpu);
	return eadr;
}
...
int zpx_adr_mode(struct _6510_cpu* cpu, char memory[][9] ){
	int zerop = zp_adr_mode(cpu,memory);
	int regx = conv_bitstr2int(cpu->regx,0,7);
	int adr = zerop + regx; 
	return adr;
}
...
int abs_adr_mode(struct _6510_cpu* cpu, char memory[][9] ){
	int cpuint = pc2int(cpu);
	int lb = conv_bitstr2int(memory[cpuint],0,7);
	int hi = conv_bitstr2int(memory[cpuint+1],0,7)<<8;
...
int abx_adr_mode(struct _6510_cpu* cpu, char memory[][9] ){
	int adr = abs_adr_mode(cpu, memory);

	return adr + conv_bitstr2int(cpu->regx,0,7);
...
int aby_adr_mode(struct _6510_cpu* cpu, char memory[][9] ){
	...
	return adr + conv_bitstr2int(cpu->regy,0,7);
...
void cpu_6502_JSR_abs(struct _6510_cpu* cpu, char mem[][9]){
	...
	cp_register(mem[pc_old], cpu->pcl);
	cp_register(mem[pc_old+1], cpu->pch);
}
...
void cpu_6502_RTS_imp(struct _6510_cpu* cpu, char mem[][9]){
	...
	cp_register(mem[sp+1], cpu->pcl);
	cp_register(mem[sp+2], cpu->pch);
	alu(ALU_OP_ADD, cpu->sp, "00000010", cpu->sp, 0);
	inc_pc(cpu);
}
